...
achievementsRouter.get("/streaks", async (req, res) => {
  try {
    const streaks = await ReadingSessionSerializer.getStreaks(req.session.profileId)
    if (streaks.length>0) {
...
achievementsRouter.get("/rank", async (req, res) => {
  try {
    ...
    const rankData = await ReadingSessionSerializer.getRankAndProgress(req.session.profileId, serializedBadges)
    return res
      .set({ "Content-Type": "application/json" })
      .status(200)
      .json({ 
